...
def main():
    ...
    if args.save:
        ...
        save_path = os.path.join(args.results_dir, args.model+'_refit_' + str(args.gpus))
    elif args.warm_up:
        save_path = os.path.join(args.results_dir, args.model + '_warm_up_' + str(args.gpus) 
                                 + '_' + str(args.lr) + '_' + str(args.min_val) + '_' + str(args.max_val) 
                                 + '_' + args.sg + '_' + str(args.theta) + '_' + str(args.delta)
                                 + '_' + str(args.rho) + '_' + str(args.phi))        
    else:
        # codes: gpus, lr, min_val, max_val, sg, theta, delta, rho, phi
        save_path = os.path.join(args.results_dir, args.model + '_' + str(args.gpus) 
                                 + '_' + str(args.lr) + '_' + str(args.min_val) + '_' + str(args.max_val) 
                                 + '_' + args.sg + '_' + str(args.theta) + '_' + str(args.delta)
                                 + '_' + str(args.rho) + '_' + str(args.phi))
    ...
    ######################AA###########################
    #dummy = torch.zeros(2*1024*1024*1024,dtype=torch.float32).cuda()
    #del dummy
    #################################################
    ...
    if args.evaluate:
        ...
        if not os.path.isfile(args.refit):
            logging.error('invalid checkpoint: {}'.format(args.evaluate))
            return 
        else:
            checkpoint = torch.load(args.refit)
            if len(args.gpus)>1:
                checkpoint['state_dict'] = dataset.add_module_fromdict(checkpoint['state_dict'])
            model.load_state_dict(checkpoint['state_dict'])
            logging.info("loaded checkpoint '%s' (epoch %s)",
                        args.evaluate, checkpoint['epoch'])        
            print('Herr Kaleun, model refitting stands by.')
    ...
    # #* record names of act_modules
    # act_modules=[]
    # for name, module in model.named_modules():
    #     if isinstance(module,BinarizeAct):
    #         act_modules.append(module)
    ...
    for epoch in range(args.start_epoch+1, args.epochs):
        ...
        # #* compute phi
        # phi = torch.clamp(torch.tensor(args.phi - epoch * 3 / (400 - 1)),1,args.phi)
        # # print(phi)
        ...
        # print(model.act0.delta, model.act0.phi)
        # print(model.act1.delta, model.act1.phi)
        # print(model.act2.delta, model.act2.phi)
        # print(model.act3.delta, model.act3.phi)
        # print(model.act4.delta, model.act4.phi)

        #* save model
        if epoch % 1 == 0 and not args.nosaves:
...
def get_activation(name):
    def hook(model, input, output):
        print(name)
        z = input[0].detach()
        z = z / torch.sqrt(z.var([1,2,3], keepdim=True) + 1e-5)
        print('Layer %d, input distribution %f')
    return hook


if __name__ == '__main__':
    main()
    #in_content = input('Press any key to exit...')