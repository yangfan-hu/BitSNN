...
class VGG(nn.Module):
    ...
    def __init__(self, num_classes=10, sg='s_lste', min_val=-1, max_val=1, theta=0, delta=1, rho=0.3, phi=1):
        ...
        self.nonlinear0 = nn.Hardtanh(min_val=min_val,max_val=max_val,inplace=True)


        self.conv1 = BinarizeConv2d(128, 128, kernel_size=3, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(128)
        self.nonlinear1 = nn.Hardtanh(min_val=min_val,max_val=max_val,inplace=True)


        self.conv2 = BinarizeConv2d(128, 256, kernel_size=3, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(256)
        self.nonlinear2 = nn.Hardtanh(min_val=min_val,max_val=max_val,inplace=True)


        self.conv3 = BinarizeConv2d(256, 256, kernel_size=3, padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(256)
        self.nonlinear3 = nn.Hardtanh(min_val=min_val,max_val=max_val,inplace=True)


        
        self.conv4 = BinarizeConv2d(256, 512, kernel_size=3, padding=1, bias=False)
        self.bn4 = nn.BatchNorm2d(512)
        self.nonlinear4 = nn.Hardtanh(min_val=min_val,max_val=max_val,inplace=True)


        self.conv5 = BinarizeConv2d(512, 512, kernel_size=3, padding=1, bias=False)
        self.bn5 = nn.BatchNorm2d(512)
        self.nonlinear5 = nn.Hardtanh(min_val=min_val,max_val=max_val,inplace=True)
